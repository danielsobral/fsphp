...
class Email
{
    ...
    {
        if (empty($this->data)) {
            $this->message->error("Erro ao enviar o e-mail, favor verifique os dados");
            return false;
        }

        if (!is_email($this->data->toEmail)) {
            $this->message->warning("O e-mail de destinatário é inválido");
            return false;
        }

        if (!is_email($fromEmail)) {
            $this->message->warning("O e-mail de remetente é inválido");
            return false;
        }
